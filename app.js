...
const OPEN_API_KEY = '76716f6677696c7336344b67715451'; // 시간표용 키를 이곳에 넣어주세요!
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchTimetable(station, requestId) {
    ...
    try {
      ...
      // 환승역 및 특수 노선(경의중앙선 등)의 정확한 역 코드 매칭 로직 개선
    ...
    const renderTimetable = (rows, isUp) => {
      ...
      // 도착/출발 시간 중 "00:00:00" 쓰레기값을 걸러내고 유효한 시간을 찾는 함수
      const getValidTime = (r) => {
        if (r.ARRIVETIME && r.ARRIVETIME !== "00:00:00") return r.ARRIVETIME;
        if (r.LEFTTIME && r.LEFTTIME !== "00:00:00") return r.LEFTTIME;
        return "00:00:00"; // 둘 다 없거나 00:00:00인 최악의 경우
      };

      let valid = rows.filter(r => timeToSeconds(getValidTime(r)) >= nowSeconds)
                      .sort((a, b) => timeToSeconds(getValidTime(a)) - timeToSeconds(getValidTime(b)))
                      .slice(0, 10);
      ...
      let html = '';
      valid.forEach(row => {
        // 필터링에 사용한 유효한 시간을 텍스트에도 동일하게 적용
        const timeStr = getValidTime(row).substring(0, 5);
